                                       ...
                                       std::size_t minimumGeometricCount)
{
    ...
    for (auto iterMap = geometricMatches.begin(); iterMap != geometricMatches.end(); ++iterMap)
    {
        const size_t photometricCount = putativeMatches.find(iterMap->first)->second.getNbAllMatches();
        const size_t geometricCount = iterMap->second.getNbAllMatches();
