                                    ...
                                    std::vector<GC_vertexInfo>& verticesAttrPrepare, mvsUtils::MultiViewParams& mp, float simFactor, float voteMarginFactor, float contributeMarginFactor, float simGaussianSize)
{
    ...
    system::parallelFor(0, cams.size(),
                        system::ParallelSettings().setThreadCount(3).setEnableNested(true),
                        [&](int c)
    {
        ...
        // Add visibility
        system::parallelFor(0, depthMap.Height(),
                            system::ParallelSettings().setDynamicScheduling(), [&](int y)
        {
